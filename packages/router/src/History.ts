...
export function createHashHistory(options: HashHistoryOptions = {}): HashHistory {
  ...
  function replace(to: To, state?: State) {
    ...
    warning(
      nextLocation.pathname.charAt(0) === "/",
      `Relative pathnames are not supported in hash history.replace(${JSON.stringify(to)})`
    );
...
export function createMemoryHistory(options: MemoryHistoryOptions = {}): MemoryHistory {
  ...
  let entries: Location[] = initialEntries.map(entry => {
    ...
    warning(
      ...
      `Relative pathnames are not supported in createMemoryHistory({ initialEntries }) (invalid entry: ${JSON.stringify(entry)})`
    );
